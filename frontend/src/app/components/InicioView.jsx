  ...
  return (
    <div className="flex-1 p-4 pb-20 md:p-8 overflow-y-auto relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 md:mb-8 gap-3">
        <div>
          <h1 className="text-[#cc679c] font-bold text-2xl md:text-4xl mb-1 md:mb-2">Panel de Inicio</h1>
          <p className="text-[#cc679c]/80 text-sm font-medium">Resumen en vivo de tu negocio</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setShowWithdrawModal(true)} className="bg-[#5db8d1] hover:bg-[#4a9bb8] text-white px-3 md:px-6 py-2.5 md:py-3 rounded-xl flex items-center gap-2 transition-all text-sm md:text-base font-bold shadow-sm">
            <PackageMinus size={18} /> <span>Retirar</span>
          </button>
          {isCajaOpen ? (
            <button onClick={() => setShowClosureModal(true)} className="bg-[#cc679c] hover:bg-[#b85889] text-[#eceae7] px-3 md:px-6 py-2.5 md:py-3 rounded-xl flex items-center gap-2 transition-all text-sm md:text-base font-medium shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-[#f4f3f0] rounded-xl p-6 border border-[#e5e7eb] relative overflow-hidden shadow-sm">
          <div className="absolute -right-4 -top-4 w-24 h-24 bg-[#cc679c]/10 rounded-full blur-xl"></div>
          <div className="flex items-center gap-4 mb-4">
            <div className="bg-[#eceae7] p-3 rounded-lg text-[#e3ac4d] shadow-inner"><Activity size={24} /></div>
            <h3 className="text-[#cc679c]/80 font-medium">Estado de Caja</h3>
          </div>
          <div className="flex items-center gap-3">
            <div className={`w-3 h-3 rounded-full ${isCajaOpen ? "bg-green-500 animate-pulse" : "bg-red-500"}`}></div>
            <p className="text-[#cc679c] text-2xl font-bold">{isCajaOpen ? "Operando" : "Cerrada"}</p>
          </div>
        </div>

        <div className="bg-[#f4f3f0] rounded-xl p-6 border border-[#5db8d1]/30 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            <div className="bg-[#5db8d1]/10 p-3 rounded-lg text-[#5db8d1]"><TrendingUp size={24} /></div>
            <h3 className="text-[#cc679c]/80 font-medium">Ventas de Hoy</h3>
          </div>
          <p className="text-[#5db8d1] text-3xl font-bold">${ventasHoy.toFixed(2)}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 flex flex-col gap-8">
          <div className="bg-[#f4f3f0] rounded-xl border border-[#e5e7eb] shadow-sm flex flex-col" style={{ height: "420px" }}>
            <div className="p-6 border-b border-[#e5e7eb] flex items-center justify-between shrink-0">
              <h2 className="text-[#5db8d1] text-xl font-bold">Actividad Reciente</h2>
              <Clock size={20} className="text-[#cc679c]/60" />
            </div>
            <div className="p-2 overflow-y-auto flex-1">
              {recentSales.length > 0 ? (
                recentSales.map((sale) => (
                  <div key={sale.id} className="flex flex-col p-4 hover:bg-[#eceae7]/50 rounded-lg transition-colors border-b border-[#e5e7eb] last:border-0">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-[#eceae7] shadow-inner flex items-center justify-center text-[#cc679c] font-bold text-xs">#{sale.id}</div>
                        <div>
                          <p className="text-[#cc679c] font-bold">Venta en salón</p>
                          <p className="text-[#cc679c]/70 font-medium text-sm">{sale.time} • {sale.items} producto{sale.items !== 1 ? "s" : ""}</p>
                        </div>
                      </div>
                      <span className="text-[#cc679c] font-black text-lg">+${sale.total.toFixed(2)}</span>
                    </div>
                    {sale.details.length > 0 && (
                      <div className="ml-14 rounded-lg p-3 border border-[#e5e7eb]" style={{ backgroundColor: "#fff" }}>
                        <p className="text-xs font-bold mb-2 uppercase tracking-wider" style={{ color: "#cc679c", opacity: 0.6 }}>Productos vendidos:</p>
                        <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
                          {sale.details.map((detail, idx) => (
                            <li key={idx} style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                              <span style={{ width: "6px", height: "6px", borderRadius: "50%", backgroundColor: "#e3ac4d", flexShrink: 0, display: "inline-block" }}></span>
                              <span style={{ fontSize: "14px", fontWeight: 600, color: "#cc679c" }}>{detail}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                ...
                <div className="flex flex-col items-center justify-center py-16 text-gray-500">
                  <Clock size={48} className="mb-4 opacity-20 text-[#cc679c]" />
                  <p className="text-lg text-[#cc679c] font-bold">{isCajaOpen ? "No hay operaciones aún" : "La caja está cerrada"}</p>
                  <p className="text-sm mt-1 text-[#cc679c]/70 font-medium">{isCajaOpen ? "Las ventas aparecerán aquí" : "Abre la caja para comenzar"}</p>
                </div>
              )}
            </div>
          </div>

          <div className="bg-[#f4f3f0] rounded-xl border border-[#e5e7eb] p-6 flex flex-col shadow-sm" style={{ height: "420px" }}>
            ...
            <div className="overflow-y-auto flex-1 space-y-4">
              {expenses.length === 0 ? (
                <p className="text-[#cc679c]/60 text-sm font-medium">Sin gastos configurados</p>
              ) : (
                expenses.map((expense) => (
                  <div key={expense.id} className="flex items-center justify-between">
                    <span className="text-[#cc679c]/80 font-medium">{expense.name}</span>
                    <span className="text-[#cc679c] font-bold">${expense.amount.toLocaleString()}</span>
                  </div>
                ))
              )}
            </div>
            <div className="mt-auto pt-6 border-t border-[#e5e7eb] shrink-0">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[#cc679c]/80 font-medium">Total a cubrir:</span>
                <span className="text-[#cc679c] font-black">${totalExpenses.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {showClosureModal && (
        <div className="fixed inset-0 bg-[#cc679c]/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#eceae7] rounded-2xl w-full max-w-md border border-[#f4f3f0] shadow-2xl">
            <div className="p-6 border-b border-[#f4f3f0]">
              <h2 className="text-[#5db8d1] font-bold text-2xl flex items-center gap-2"><Lock size={24} className="text-[#cc679c]" /> Cierre de Caja</h2>
            </div>
            <div className="p-6">
              <p className="text-[#cc679c]/80 font-medium">¿Estás seguro que deseas cerrar la caja? Para ver el resumen detallado dirígete a la pestaña "Caja".</p>
            </div>
            <div className="p-6 border-t border-[#f4f3f0] flex gap-4">
              ...
              <button onClick={handleConfirmClose} className="flex-1 bg-[#cc679c] hover:bg-[#b85889] text-[#eceae7] font-bold py-4 rounded-xl transition-all shadow-md">Confirmar Cierre</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-[#cc679c]/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#eceae7] rounded-2xl w-full max-w-md border border-[#f4f3f0] shadow-2xl">
            <div className="p-6 border-b border-[#f4f3f0] flex items-center justify-between">
              <h2 className="text-[#5db8d1] font-bold text-2xl flex items-center gap-2"><PackageMinus size={24} className="text-[#e3ac4d]" /> Retirar Uso Interno</h2>
            ...
            <form onSubmit={handleWithdraw} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white text-[#cc679c] placeholder-[#cc679c]/50 rounded-xl px-4 py-3 mb-3 border border-[#f4f3f0] focus:border-[#cc679c] focus:ring-2 focus:ring-[#cc679c]/20 outline-none font-bold shadow-sm"
                />
                <select
                  value={withdrawProductId}
                  onChange={(e) => setWithdrawProductId(e.target.value)}
                  className="w-full bg-white text-[#cc679c] rounded-xl px-4 py-3 border border-[#f4f3f0] focus:border-[#cc679c] outline-none font-bold shadow-sm"
                  required
                >
              ...
              <div className="pt-4 flex gap-4">
                <button type="button" onClick={() => { setShowWithdrawModal(false); setSearchWithdraw(""); setWithdrawQty(""); }} className="flex-1 bg-[#f4f3f0] hover:bg-[#e5e7eb] text-[#cc679c] font-bold py-4 rounded-xl transition-all shadow-sm">Cancelar</button>
                <button type="submit" disabled={submittingWithdraw || !withdrawProductId || !withdrawQty} className="flex-1 bg-[#cc679c] hover:bg-[#b85889] disabled:bg-[#f4f3f0] disabled:text-[#cc679c]/50 text-[#eceae7] font-bold py-4 rounded-xl transition-all shadow-md">{submittingWithdraw ? "Retirando..." : "Confirmar Retiro"}</button>
              </div>
            </form>
          </div>
        </div>
